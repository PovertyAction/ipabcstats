...
cap program drop ipacheckimport_bc
cap program drop dropexcess
cap program drop checkrequiredcommands

program ipacheckimport_bc, rclass
			...
			if `"`sheet'"' == "0. setup" {
				...
				*<!> TO ADD => replacements file, master tracking list, tracking globals
				local boxes =                                 ///
					...
					`""Backcheck Date Variable""'						+ ///
					`""Show Unique IDs with Threshold Errors \(opt\.\)""'	+ ///
					`""Include All Comparisons \(opt\.\)""'					+ ///
					`""Do not Use Value labels for Factors \(opt\.\)""'		+ ///
					`""Replace Back Check Comparison File \(opt\.\)""'		+ ///
					`""Ignore Difference in Numeric Variables \(opt\.\)""'	+ ///
					`""Ignore Difference in String Variables \(opt\.\)""'	+ ///
					`""Exclude Missing Values from Comparison \(opt\.\)""'	+ ///
					`""Exclude Numeric Values from Comparison \(opt\.\)""'	+ ///
					`""Exclude String Values from Comparison \(opt\.\)""'	+ ///
				...
				if inlist(`"`sheet'"', "backchecks") {
					if `rows' > 0 {
						...
						* initialize Stata global
						...
						mata: st_global("prtest`n'", "")
						mata: st_global("srtest`n'", "")
						...
						loc varcount `r(N)'
						tostring type test reliability okrange_min okrange_max, replace
						replace okrange_max = "" if okrange_max == "."
						replace okrange_min = "" if okrange_min == "."
						...
						forval i = 1/`rows' {
							if `i' <= `varcount' {
								...
								if test[`i'] == "prtest" {
									mata: st_global("prtest`n'", `"${ttest`n'} `=variable[`i']'"')
								}								
								if test[`i'] == "signrank" {
									mata: st_global("srtest`n'", `"${ttest`n'} `=variable[`i']'"')
								}
